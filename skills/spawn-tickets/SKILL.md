...
For each one, build an item whose prompt just tells the agent to read that ticket by path or issue reference and do the work — nothing copied from the ticket itself. Call the Skill tool for `spawn-agents` with the resulting item list and let it handle confirmation, worktrees, and spawning.
